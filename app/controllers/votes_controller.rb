      ...
      if votable_type == "Question"
        format.turbo_stream do
          render turbo_stream: turbo_stream.append('flash-messages', partial: 'shared/notifications', locals: {message: flash[:notice]}) +
                              turbo_stream.replace("votes_question", partial: 'votes/votes_question', locals: {question: @question })
        end
      elsif votable_type == "Answer"
        set_answer
        format.turbo_stream do
          render turbo_stream: turbo_stream.append('flash-messages', partial: 'shared/notifications', locals: {message: flash[:notice]}) +
                              turbo_stream.replace("votes_answer_#{@answer.id}", partial: 'votes/votes_answers', locals: {question: @question, answer: @answer})
        end
      end
    end
  end
